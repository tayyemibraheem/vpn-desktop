...
#[tauri::command]
async fn vpn_connect(app: tauri::AppHandle, state: State<'_, AppState>, username: String, password: String) -> Result<serde_json::Value, ()> {
...
async fn split_tunnel_set_config(state: State<'_, AppState>, config: store::SplitTunnelConfig) -> Result<(), ()> {
    ...
    if state.ovpn.status().0 == "connected" {
        if let Some(physical) = state.ovpn.physical_gateway() {
            destination_routes::clear_all(&state.ovpn.applied_destination_routes());
            let applied = destination_routes::apply(&config.destinations, &physical).await;
            state.ovpn.set_applied_destination_routes(applied);
...
fn main() {
    ...
    tauri::Builder::default()
        ...
        .manage(AppState {
            ...
            ovpn: Arc::new(ovpn::OvpnClient::new()),
